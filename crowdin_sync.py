...
def clean_file(base_path, project_path, filename):
    ...
    for c in comments:
        p = c.getparent()
        if p is None:
            # Keep copyright header, which is always outside of the root tags (therefore no parent)
            # and contains the string 'Copyright (C)'
            comment = str(c)
            if 'copyright (c)' in comment.lower() or 'crowdin.com' in comment.lower():
                copyright_comment += comment.replace('\\n', '\n') + '\n'
            continue
        p.remove(c)
    ...
    if copyright_comment != '':
        content = content.replace('?>\n', '?>\n' + copyright_comment)

    # Sometimes these spaces are added, we don't want them
