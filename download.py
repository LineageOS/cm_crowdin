...
def upload_translations_gerrit(extracted, xml, base_path, branch, username):
    ...
    for file_path in extracted:
        ...
        if best_match_path:
            ...
            print(f"WARNING: Could not find a matching project in XML for file: [{file_path}], skipping.")
    ...
    for project_path, files_in_project in projects_to_push.items():
        ...
        print(f"Processing project: {project_name} (path: {project_path}) with {len(files_in_project)} files.")
        push_as_commit(
            files_in_project,
            base_path,
            project_path,
