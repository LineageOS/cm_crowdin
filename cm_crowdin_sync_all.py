...
proc = subprocess.Popen(['crowdin-cli -c crowdin_cm.yaml list sources && crowdin-cli -c crowdin_aosp.yaml list sources'], stdout=subprocess.PIPE, shell=True)
            ...
            for project_item in items:
                ...
                if project_item.attributes['path'].value == good_path:
                    if project_item.hasAttribute('revision'):
                        ...
                        branch = 'cm-11.0'
                    print('Committing ' + project_item.attributes['name'].value + ' on branch ' + branch + ' (based on android/default.xml or crowdin/extra_packages.xml)')
